...
public class StackEditor implements PlugIn {
	...
	public void run(String arg, String format, Path outputPath, ImagePlus imp) {
	...
	public void convertStackToImages(ImagePlus imp) {
		...
		for (int i=1; i<=size; i++) {
			...
			String temp = info.substring(title.lastIndexOf("/")+1).replaceAll("\\s+","");
			String titleClean = addPreffix(temp, i+"-");
			String fileUnstackName= outputPath.resolve(titleClean).toString();
			IJ.saveAs(imp2, format, fileUnstackName);
		}
		imp.changes = false;
	...
	private String addPreffix(String str, String preffix) {
		StringBuilder sb = new StringBuilder();
		sb.append(preffix).append(str);
		return sb.toString();
	}

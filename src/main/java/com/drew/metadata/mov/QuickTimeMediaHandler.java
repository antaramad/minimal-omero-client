...
public abstract class QuickTimeMediaHandler<T extends QuickTimeDirectory> extends QuickTimeHandler<T>
{
    ...
    public boolean shouldAcceptAtom(@NotNull Atom atom)
    {
        return atom.type.equals(getMediaInformation())
            || atom.type.equals(com.drew.metadata.mov.QuickTimeAtomTypes.ATOM_SAMPLE_DESCRIPTION)
            || atom.type.equals(com.drew.metadata.mov.QuickTimeAtomTypes.ATOM_TIME_TO_SAMPLE);
    }

    @Override
    public boolean shouldAcceptContainer(@NotNull Atom atom)
    {
        return atom.type.equals(com.drew.metadata.mov.QuickTimeContainerTypes.ATOM_SAMPLE_TABLE)
            || atom.type.equals(com.drew.metadata.mov.QuickTimeContainerTypes.ATOM_MEDIA_INFORMATION)
    ...
    public QuickTimeMediaHandler<?> processAtom(@NotNull Atom atom, @Nullable byte[] payload, QuickTimeContext context) throws IOException
    {
        if (payload != null) {
            ...
            } else if (atom.type.equals(com.drew.metadata.mov.QuickTimeAtomTypes.ATOM_SAMPLE_DESCRIPTION)) {
